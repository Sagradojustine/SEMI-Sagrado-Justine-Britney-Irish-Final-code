...
// Subjects PDF Document
const SubjectsPDF = ({ subjects, title }) => (
  <Document>
      ...
      <View style={styles.table}>
        <View style={styles.tableRow}>
          <View style={styles.tableColHeader}>
            <Text style={styles.tableCellHeader}>Subject Code</Text>
          </View>
          <View style={styles.tableColHeader}>
        ...
        {subjects.map((subject, index) => (
          <View style={styles.tableRow} key={index}>
            <View style={styles.tableCol}>
              <Text style={styles.tableCell}>{subject.subject_code}</Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableCell}>{subject.subject_name}</Text>
            </View>
            <View style={styles.tableCol}>
              <Text style={styles.tableCell}>{subject.instructor}</Text>
            </View>
          </View>
        ))}
      </View>
...
const SubjectsPage = () => {
  ...
  const generatePDF = async () => {
    try {
      const blob = await pdf(<SubjectsPDF subjects={filteredSubjects} title="Subjects Report" />).toBlob();
...
// SubjectForm Component (unchanged)
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
      <div className="bg-gradient-to-br from-white via-pink-50 to-fuchsia-50 rounded-3xl p-8 w-full max-w-md shadow-2xl border-4 border-pink-400 relative transform animate-scale-in">
        ...
        <h2 className="text-2xl font-extrabold mb-6 bg-gradient-to-r from-pink-600 to-fuchsia-600 text-transparent bg-clip-text flex items-center gap-2">
          {subject ? '✏️ Edit Subject' : '➕ Add New Subject'}
        </h2>
        
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-sm font-bold text-pink-700 mb-2">Subject Code</label>
            <input
              type="text"
              name="subject_code"
          ...
          <div>
            <label className="block text-sm font-bold text-pink-700 mb-2">Subject Name</label>
            <input
              type="text"
              name="subject_name"
          ...
          <div>
            <label className="block text-sm font-bold text-pink-700 mb-2">Instructor</label>
            <input
              type="text"
              name="instructor"
